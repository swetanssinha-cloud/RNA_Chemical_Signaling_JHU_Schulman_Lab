...
def simulate_sender_receiver_animated(params: SenderReceiverParams, 
                                     output_path: Path = None,
                                     frame_interval_steps: int = 10,
                                     animation_interval_ms: int = 100,
                                     verbose: bool = True):
    """
    Simulate sender/receiver and create an animation of the diffusion process.
    
    Args:
        params: Simulation parameters
        output_path: Path to save animation (e.g., 'animation.gif' or 'animation.mp4')
        frame_interval_steps: Capture a frame every N simulation steps
        animation_interval_ms: Time between frames in milliseconds
    ...
    ax_i2 = fig.add_subplot(gs[0, 1])
    ax_total = fig.add_subplot(gs[0, 2])
    
    # Time series plots
    ax_i2_time = fig.add_subplot(gs[1, :])
    ax_rna_time = fig.add_subplot(gs[2, :])
    
    # Prepare receiver outline for contour
    receiver_outline = field_to_image(receiver_mask.astype(float), nx, ny)
    
    # Initialize field images
    s2_data = field_to_image(np.asarray(vars_by_name["S2"].value) / NANOMOLAR, nx, ny)
    i2_data = field_to_image(np.asarray(vars_by_name["I2"].value) / NANOMOLAR, nx, ny)
    total_rna_data = field_to_image(
        (np.asarray(vars_by_name["S2"].value)
         + np.asarray(vars_by_name["S2_I2"].value)
         + np.asarray(vars_by_name["S2_Th2"].value)) / NANOMOLAR,
        nx, ny
    )
    
    # Determine color scale limits (will update dynamically)
    s2_max = 1.0  # Will be updated
    i2_max = params.receiver_switch_nM
    total_max = 1.0  # Will be updated
    
    im_s2 = ax_s2.imshow(s2_data, origin="lower", cmap="viridis", vmin=0, vmax=s2_max)
    ax_s2.contour(receiver_outline, levels=[0.5], colors="white", linewidths=0.8)
    ax_s2.set_title("Free S2 (nM)")
    ax_s2.set_xticks([])
    ax_s2.set_yticks([])
    cbar_s2 = plt.colorbar(im_s2, ax=ax_s2, fraction=0.046, pad=0.04)
    
    im_i2 = ax_i2.imshow(i2_data, origin="lower", cmap="plasma", vmin=0, vmax=i2_max)
    ax_i2.contour(receiver_outline, levels=[0.5], colors="white", linewidths=0.8)
    ax_i2.set_title("I2 (nM)")
    ax_i2.set_xticks([])
    ax_i2.set_yticks([])
    cbar_i2 = plt.colorbar(im_i2, ax=ax_i2, fraction=0.046, pad=0.04)
    
    im_total = ax_total.imshow(total_rna_data, origin="lower", cmap="inferno", vmin=0, vmax=total_max)
    ax_total.contour(receiver_outline, levels=[0.5], colors="white", linewidths=0.8)
    ax_total.set_title("Total RNA (nM)")
    ax_total.set_xticks([])
    ax_total.set_yticks([])
    cbar_total = plt.colorbar(im_total, ax=ax_total, fraction=0.046, pad=0.04)
    
    # Time series plots
    line_i2, = ax_i2_time.plot(times_h, receiver_i2_nM, color="#0c5da5", lw=2)
    ax_i2_time.set_ylabel("Receiver I2 (nM)")
    ax_i2_time.set_xlim(0, params.total_hours)
    ax_i2_time.set_ylim(0, params.receiver_switch_nM * 1.1)
    ax_i2_time.grid(alpha=0.25)
    ax_i2_time.axvline(0, color='red', linestyle='--', alpha=0.5, linewidth=1)
    
    line_rna, = ax_rna_time.plot(times_h, receiver_total_rna_nM, color="#b54e00", lw=2)
    ax_rna_time.set_xlabel("Time (h)")
    ax_rna_time.set_ylabel("Receiver total RNA (nM)")
    ax_rna_time.set_xlim(0, params.total_hours)
    ax_rna_time.set_ylim(0, 10)  # Will adjust if needed
    ax_rna_time.grid(alpha=0.25)
    ax_rna_time.axvline(0, color='red', linestyle='--', alpha=0.5, linewidth=1)
    
    time_marker_i2 = ax_i2_time.axvline(0, color='red', linestyle='--', alpha=0.7, linewidth=1.5)
    time_marker_rna = ax_rna_time.axvline(0, color='red', linestyle='--', alpha=0.7, linewidth=1.5)
    
    fig.suptitle(f"Sender/Receiver Simulation | Distance = {params.center_distance_um:.0f} μm | Time = 0.00 h", 
                 fontsize=14, fontweight='bold')
    
    # Animation update function
    def update(frame):
        # Perform simulation steps
        steps_to_run = frame_interval_steps if frame > 0 else 0
        ...
        for _ in range(steps_to_run):
            ...
            current_time_h = (len(times_h)) * frame_interval_steps * params.dt_s / 3600.0
            times_h.append(current_time_h)
            receiver_i2_nM.append(mean_in_mask(vars_by_name["I2"], receiver_mask) / NANOMOLAR)
            receiver_total_rna_nM.append((
                mean_in_mask(vars_by_name["S2"], receiver_mask)
                + mean_in_mask(vars_by_name["S2_I2"], receiver_mask)
                + mean_in_mask(vars_by_name["S2_Th2"], receiver_mask)
            ) / NANOMOLAR)
        ...
        s2_max_current = max(s2_data.max(), 1.0)
        total_max_current = max(total_rna_data.max(), 1.0)
        
        im_s2.set_array(s2_data)
        im_s2.set_clim(0, s2_max_current)
        
        im_i2.set_array(i2_data)
        
        im_total.set_array(total_rna_data)
        im_total.set_clim(0, total_max_current)
        
        # Update time series
        line_i2.set_data(times_h, receiver_i2_nM)
        line_rna.set_data(times_h, receiver_total_rna_nM)
        
        # Adjust y-limits if needed
        if receiver_total_rna_nM[-1] > ax_rna_time.get_ylim()[1] * 0.9:
            ax_rna_time.set_ylim(0, max(receiver_total_rna_nM) * 1.1)
        
        # Update time markers
        current_time = times_h[-1]
        time_marker_i2.set_xdata([current_time, current_time])
        time_marker_rna.set_xdata([current_time, current_time])
        
        # Update title
        fig.suptitle(f"Sender/Receiver Simulation | Distance = {params.center_distance_um:.0f} μm | Time = {current_time:.2f} h", 
                     fontsize=14, fontweight='bold')
        
        if verbose and frame % max(1, (n_steps // frame_interval_steps) // 10) == 0:
            print(f"Frame {frame}/{n_steps // frame_interval_steps} | t = {current_time:.2f} h | "
                  f"receiver I2 = {receiver_i2_nM[-1]:.3f} nM | "
                  f"receiver total RNA = {receiver_total_rna_nM[-1]:.3f} nM")
        
        return [im_s2, im_i2, im_total, line_i2, line_rna, time_marker_i2, time_marker_rna]
    
    # Create animation
    n_frames = n_steps // frame_interval_steps
    ...
    if output_path:
        ...
        if str(output_path).endswith('.gif'):
            anim.save(output_path, writer='pillow', fps=1000//animation_interval_ms)
        else:
            anim.save(output_path, writer='ffmpeg', fps=1000//animation_interval_ms, dpi=100)
        print(f"Animation saved to {output_path}")
    
    return anim, fig, {
        "times_h": np.array(times_h),
        "receiver_i2_nM": np.array(receiver_i2_nM),
        "receiver_total_rna_nM": np.array(receiver_total_rna_nM),
    }
...
def simulate_sender_receiver(params: SenderReceiverParams, verbose: bool = True):
    ...
    for step in range(1, n_steps + 1):
        ...
        while residual > params.nonlinear_tolerance and sweep_count < params.max_sweeps_per_step:
            ...
            residual = max(residual, eqs["I2"].sweep(var=vars_by_name["I2"], dt=params.dt_s))
            residual = max(
                residual, eqs["S2_I2"].sweep(var=vars_by_name["S2_I2"], dt=params.dt_s)
            )
            residual = max(residual, eqs["Th2"].sweep(var=vars_by_name["Th2"], dt=params.dt_s))
            residual = max(
                residual, eqs["S2_Th2"].sweep(var=vars_by_name["S2_Th2"], dt=params.dt_s)
            )
            clip_nonnegative(vars_by_name)
            sweep_count += 1
...
def parse_args():
    ...
    parser.add_argument("--animate", action="store_true", help="Create animation of the simulation.")
    parser.add_argument("--animation-output", type=Path, default=None, 
                       help="Path to save animation (e.g., 'animation.gif' or 'animation.mp4')")
    parser.add_argument("--frame-interval", type=int, default=10, 
                       help="Capture animation frame every N simulation steps.")
    parser.add_argument("--animation-speed", type=int, default=100, 
...
def main():
    ...
    if args.animate:
        # Create animation
        animation_path = args.animation_output or args.output_prefix.with_name(
            args.output_prefix.name + "_animation.gif"
        )
        anim, fig, data = simulate_sender_receiver_animated(
            params,
            output_path=animation_path,
            frame_interval_steps=args.frame_interval,
            animation_interval_ms=args.animation_speed,
            verbose=not args.quiet
        )
        plt.show()
    else:
        # Original static simulation
    ...
    if args.sweep_distances_um and not args.animate:
        final_i2 = []
        final_total_rna = []
