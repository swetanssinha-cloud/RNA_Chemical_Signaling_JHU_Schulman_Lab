'''This is sweep any parameter but with the triangular mesh. 

The problem before is that the triangular mesh would be overwritten as multiprocessing tried to 
create a new Mesh every time. 

The solution is to create the mesh in sequence, and then run the solvers in parallel. 
'''
"""
...
DEFAULT_PARAMS = {
    ...
    'total_height': 1e3,
    'dt': 30.0,
    'total_time': 1 * 3600, #testing one hour first so I can actually get some results
...
def run_single_simulation(param_value, replicate_id=0):
    ...
    try:
        ...
        mesh_filename = get_mesh_filename(params)
            ...
            # Check if mesh needs to be created (for non-geometry parameters)
        if not Path(mesh_filename).exists():
                    # This should only happen for Th2, k_p, etc. sweeps
                    ...
                    sender_center_y = total_height / 2.0
...
def run_parameter_sweep():
    """Smart parameter sweep with conditional mesh pre-generation"""
    ...
    # Check if we need different meshes
    if determine_if_mesh_changes(SWEEP_PARAMETER):
        print(f"⚠️  '{SWEEP_PARAMETER}' affects mesh geometry")
        print("Pre-generating all unique meshes to avoid Gmsh conflicts...")
        print("="*80)
        ...
        for param_value in SWEEP_VALUES:
            ...
            if not Path(mesh_filename).exists():
                print(f"Creating: {mesh_filename}")
                _ = create_gmsh_radial_mesh(
        ...
        print("✓ All meshes pre-generated\n")
    else:
        print(f"✓ '{SWEEP_PARAMETER}' doesn't affect mesh geometry")
        print("All simulations will use the same mesh (fast!)")
        print("="*80 + "\n")
    
    # Now run parallel simulations
