"""
Shared model, solver, and sweep orchestration for the 2-node tethered-genelet
sender/receiver diffusion model.

Every per-parameter sweep script in this folder (sweep_k_d_ds.py,
sweep_distance_between.py, ...) imports from here and only specifies WHAT to
vary, via a SweepConfig. Changing the solver, the mesh geometry, the
validation gates, or the plotting code here changes every sweep at once --
there is exactly one copy of each, so it cannot drift out of sync between
per-parameter scripts the way copy-pasted versions would.
...
MESH_DIR = SWEEP_ROOT / "meshes_conformal"
...
DEFAULT_PARAMS = {
    ...
    # time stepping
    "dt": 60.0,                 # s
    "total_time": 48 * 3600,    # s -- 16 h hard stop. Steady-state detection #48 hours not for all cases
...
def run_single_simulation(cfg, param_value, replicate_id):
    ...
    params = apply_sweep_value(cfg, param_value)
    ...
    # by running it again.
    existing = timeseries_path_for(cfg, param_value, replicate_id, params)
    if existing.exists():
    ...
    try:
        ...
        ts_file = timeseries_path_for(cfg, param_value, replicate_id, params)
...
def build_all_meshes(cfg):
    ...
    if cfg.sweep_parameter in MESH_AFFECTING:
        values = cfg.sweep_values
        print(f"'{cfg.sweep_parameter}' changes the geometry -> "
              f"{len(values)} mesh(es) needed.")
    else:
        values = [DEFAULT_PARAMS[cfg.sweep_parameter]]
        print(f"'{cfg.sweep_parameter}' does not change the geometry -> "
              f"1 shared mesh.")

    wanted = []
    for value in values:
        params = dict(DEFAULT_PARAMS)
        params[cfg.sweep_parameter] = value
...
def collect_results_from_disk(cfg):
    ...
    pattern = re.compile(
        rf"timeseries_{re.escape(cfg.sweep_parameter)}="
        rf"(?P<value>[-+0-9.eE]+)_rep=(?P<rep>\d+)")

    rows = []
    for path in sorted(cfg.output_dir.glob(f"timeseries_{cfg.sweep_parameter}=*_rep=*.csv")):
...
def summarise(cfg):
    ...
    check_parameter_had_an_effect(cfg, raw)

    return stats


def check_parameter_had_an_effect(cfg, raw):
    """
    ...
    if spread / scale < 1e-12:
        ...
        print(f"WARNING: every run returned the same {key} "
              f"({values[0]:.12g}).")
        print(f"'{cfg.sweep_parameter}' varied across {raw['param_value'].nunique()} "
        ...
        print(f"params['{cfg.sweep_parameter}'] and not a different key.")
        print("!" * 78)
...
def plot(cfg, stats):
    ...
    fig.suptitle(f"Parameter sweep: {cfg.sweep_parameter}",
    ...
    for ax in axes.flat:
        ax.set_xlabel(cfg.sweep_parameter)
        ax.grid(alpha=0.3)
...
def run(cfg):
    ...
    if cfg.sweep_parameter not in DEFAULT_PARAMS:
        raise SystemExit(
