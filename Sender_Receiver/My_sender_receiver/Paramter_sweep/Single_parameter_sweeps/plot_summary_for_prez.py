...
It also writes a leaner, 2-row x 1-column version with only the two metrics
that matter most for a slide -- half-time (top) and final [I2] (bottom) --
as summary_I2_halftime_for_prez.png, alongside the 4-panel summary_for_prez.png
(not replacing it).
...
def series(ax, xs, stats, metric, color, title, ylabel):
...
def plot_summary(folder_name):
    ...
    series(axes[0, 0], xs, stats, "half_time_center_hr", "tab:green",
           "Turn-on time", "Half-time of I2 (hours)")
    series(axes[0, 1], xs, stats, "I2_center_final_nM", "tab:blue",
           "Receiver switch", "Final [I2] at receiver (nM)")
    series(axes[1, 0], xs, stats, "S2_free_center_final_nM", "tab:orange",
           "Free RNA remaining", "Final free [S2] at receiver (nM)")
    series(axes[1, 1], xs, stats, "S2_total_center_final_nM", "tab:purple",
...
def plot_summary_two_panel(folder_name):
    folder = HERE / folder_name
    stats, sweep_parameter = load_summary(folder)
    xs = stats["param_value"].to_numpy()
    ...
    series(ax_half, xs, stats, "half_time_center_hr", "tab:green",
           "Turn-on time", "Half-time of I2 (hours)")
    series(ax_i2, xs, stats, "I2_center_final_nM", "tab:blue",
           "Receiver switch", "Final [I2] at receiver (nM)")
    ...
    fig.tight_layout()
    out = folder / "summary_I2_halftime_for_prez.png"
    fig.savefig(out, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"Wrote {out}")
...
if __name__ == "__main__":
    for folder_name in FOLDERS:
        ...
        plot_summary_two_panel(folder_name)
