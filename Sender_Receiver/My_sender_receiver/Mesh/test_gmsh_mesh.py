...
# mesh_filename, sender_x, receiver_x, y_ctr = create_gmsh_radial_mesh(
#     bath_width=10000.0,       # 1 cm
#     bath_height=1000.0,       # 1 mm  
#     node_diameter=75.0,       # 75 μm nodes
#     distance_between_nodes=1000.0,
...
mesh_filename = "preview_ccd=1200_v3.msh"
mesh = Gmsh2D(mesh_filename)
y_ctr = (1e3)/2
sender_x = (1e4)/2 - (1200)/2
receiver_x = sender_x + 1200
...
for i in range(mesh.numberOfFaces):
    ...
    plt.plot(triangle_x, triangle_y, 'k-', linewidth=0.2, alpha=0.4)
...
# plt.add_patch(node_circle_sender)
# plt.add_patch(node_circle_receiver)
# plt.set_xlim([sender_x - 500, receiver_x + 500])
# plt.set_ylim([y_ctr - 300, y_ctr + 300])
# plt.set_aspect('equal')
# plt.set_title('Full Triangular Mesh Structure', fontsize=14, fontweight='bold')
# plt.set_xlabel('X Position (μm)')
# plt.set_ylabel('Y Position (μm)')
...
plt.tight_layout()
plt.savefig('mesh_validation_visual_v3.png', dpi=300)
