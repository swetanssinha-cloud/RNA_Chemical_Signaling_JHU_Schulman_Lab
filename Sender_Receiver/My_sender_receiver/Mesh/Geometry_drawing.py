...
tube_width = 5000  # μm
tube_height = 5000  # μm
center_distance = 1200  # μm
node_radius = 75/2  # μm
display_node_radius = node_radius * 6  # μm (visual only, exaggerated for visibility)
margin = 600  # μm (visual only)

# Calculate positions
view_left = 0
view_right = tube_width
sender_x = (tube_width - center_distance) / 2
sender_y = tube_height / 2
receiver_x = sender_x + center_distance
receiver_y = tube_height / 2

# Draw the four sides of the tube as a closed box
ax.plot([view_left, view_right],
...
ax.plot([view_left, view_left],
        [0, tube_height], 'k-', linewidth=2.5, solid_capstyle='butt', clip_on=False)
ax.plot([view_right, view_right],
        [0, tube_height], 'k-', linewidth=2.5, solid_capstyle='butt', clip_on=False)

# Draw sender node (blue circle)
sender_circle = patches.Circle((sender_x, sender_y), display_node_radius,
...
receiver_circle = patches.Circle((receiver_x, receiver_y), display_node_radius,
...
ax.text(sender_x, sender_y - display_node_radius - 60, 'Sender',
        ha='center', va='top', fontsize=14, color='blue', fontweight='bold')
ax.text(receiver_x, receiver_y - display_node_radius - 60, 'Receiver',
...
# Sender radius label
ax.text(sender_x - display_node_radius - 180, sender_y + 150,
        f'radius\n= {node_radius} μm',
...
ax.text(receiver_x + display_node_radius + 180, receiver_y + 150,
        f'radius\n= {node_radius} μm',
...
# Caption noting nodes are drawn oversized for visibility
ax.text((view_left + view_right) / 2, -280,
        '(Nodes bigger for demonstration purposes)',
...
info_text = (
    ...
    f'  • Tube width     = {tube_width} μm\n'
    f'  • Tube height    = {tube_height} μm\n'
