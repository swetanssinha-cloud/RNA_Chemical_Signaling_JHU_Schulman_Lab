...
MODIFIED: Uses fixed domain size to avoid boundary effects when sweeping center_distance_um
"""
...
OUTPUT_DIR = Path("sweep_results_fixed_domain")
...
# ============================================================================
# FIXED DOMAIN SIZE CONFIGURATION
# This prevents boundary effects when sweeping center_distance_um
# Domain is sized to accommodate the maximum distance in the sweep
FIXED_DOMAIN_WIDTH_UM = 2500.0  # Large enough for max distance + margins
...
def build_geometry(params: SenderReceiverParams):
    """
    Build mesh with FIXED domain size to avoid boundary effects during parameter sweeps.
    
    Uses FIXED_DOMAIN_WIDTH_UM for width instead of scaling with center_distance_um.
    This ensures all simulations have the same boundary conditions.
    """
    # FIXED width for all simulations
    width_um = FIXED_DOMAIN_WIDTH_UM
...
def run_parameter_sweep(sweep_param: str, sweep_values: list, output_dir: Path, 
    ...
    print(f"Fixed domain width: {FIXED_DOMAIN_WIDTH_UM} μm")
