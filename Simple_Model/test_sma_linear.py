...
def run_simulation(Phi_in_value, params):
    ...
    NO DEGRADATION VERSION (k_d_ss = k_d_ds = 0)
    """
    # Unpack parameters
    ...
    # Define equations WITHOUT degradation terms
...
ax1.set_title('Free Signal S2 vs Time (NO DEGRADATION)', fontsize=14)
...
ax2.set_title('Total Signal (S2 + C_I2 + C_Th2) vs Time (NO DEGRADATION)', fontsize=14)
...
ax3.set_title('Receiver I2 vs Time (NO DEGRADATION)', fontsize=14)
...
plt.title('Time for I2 to Drop to 50 nM vs Input Flux (NO DEGRADATION)', fontsize=14)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()

# Print final steady-state values
print("\nFinal values at t=8h (NO DEGRADATION):")
for Phi_in_val in Phi_in_array:
    time, S2, I2, tw50, S2_tot = results[Phi_in_val]
...
# Verify linearity: check if S2_total = Phi_in * time
print("\n=== Verification: Is S2_total linear? ===")
for Phi_in_val in Phi_in_array[:3]:  # Check first 3 values
    time, S2, I2, tw50, S2_tot = results[Phi_in_val]
    expected = Phi_in_val * time * 1e9  # Expected: Phi_in * t (in nM)
    actual = S2_tot * 1e9
    print(f"\nΦ_in = {Phi_in_val*1e9:.2f} nM/s:")
    print(f"  At t=4h: Expected={expected[240]:.1f} nM, Actual={actual[240]:.1f} nM")
    print(f"  At t=8h: Expected={expected[-1]:.1f} nM, Actual={actual[-1]:.1f} nM")