...
def run_simulation(k_d_ss, params):
    """
    Run well-mixed FiPy simulation for given k_d_ss value.
    I want to test system with different kill rates for single stranded RNA S2 (I think)
...
# Array of k_d_ss to sweep (in 1/s)
k_d_ss_array = np.array([1, 2, 3, 4, 5]) * 3e-4  # 1/s
print("k_d_ss values to sweep (in 1/s):", k_d_ss_array)
...
for k_d_ss in k_d_ss_array:
    print(f"Running simulation for k_d_ss = {k_d_ss:.2e} 1/s...")
    time, S2, I2, tw50, S2_tot = run_simulation(k_d_ss, params)
    results[k_d_ss] = (time, S2, I2, tw50, S2_tot)

    # Collect tw50 data for plotting
    k_d_ss_values.append(k_d_ss)
...
for k_d_ss in k_d_ss_array:
    time, S2, I2, tw50, S2_tot = results[k_d_ss]
    ax1.plot(time / 3600, S2 * 1e9, 
             label=f'k_d_ss = {k_d_ss:.5f} 1/s', 
             linewidth=2)
...
plt.plot(np.array(k_d_ss_values)[valid_indices], 
         np.array(tw50_values)[valid_indices] / 3600,  # Convert to hours
         'o-', linewidth=2, markersize=8)
plt.xlabel('Decay Rate k_d_ss (1/s)', fontsize=12)
plt.ylabel('Time to [I2] = 50 nM (hours)', fontsize=12)
plt.title('Time for I2 to Drop to 50 nM vs Decay Rate', fontsize=14)
...
for k_d_ss in k_d_ss_array:
    ...
    print(f"  k_d_ss = {k_d_ss:.5f} 1/s → [S2 free] = {S2[-1]*1e9:.1f} nM, "
