...
def run_simulation(k_d_ds, params):
    """
    Run well-mixed FiPy simulation for given k_d_ds value.
...
params = {
    ...
    'n_steps': 480,     # 8 hours
    'max_sweeps': 3,    # keep as 20 for larger system maybe, but for this small system 3 is enough
    'tol': 1e-10,
    'Phi_in': 0.02      # M/s (0.2 nM/s)
}
...
# Array of k_d_ds to sweep (in 1/s)
k_d_ds_array = np.array([1, 2, 3, 4, 5]) * 3e-4
print("k_d_ds values to sweep (in 1/s):", k_d_ds_array)
...
for k_d_ds in k_d_ds_array:
    print(f"Running simulation for k_d_ds = {k_d_ds:.2e} 1/s...")
    t, S2, I2, tw50, S2_tot = run_simulation(k_d_ds, params)
    results[k_d_ds] = (t, S2, I2, tw50, S2_tot)

    # Collect tw50 data for plotting
    k_d_ds_values.append(k_d_ds)
...
for k_d_ds in k_d_ds_array:
    t, S2, I2, tw50, S2_tot = results[k_d_ds]
    ax1.plot(t / 3600, S2 * 1e9, 
             label=f'k_d_ds = {k_d_ds:.5f} 1/s', 
             linewidth=2)

ax1.set_xlabel('Time (hours)', fontsize=12)
ax1.set_ylabel('[S2 free] (nM)', fontsize=12)
ax1.set_title('Free Signal S2 vs Time for Different Decay Rates k_d_ds', fontsize=14)
...
for k_d_ds in k_d_ds_array:
    t, S2, I2, tw50, S2_tot = results[k_d_ds]
    ax3.plot(t / 3600, I2 * 1e9, 
             label=f'k_d_ds = {k_d_ds:.5f} 1/s', 
             linewidth=2)

ax3.set_xlabel('Time (hours)', fontsize=12)
ax3.set_ylabel('[I2] (nM)', fontsize=12)
ax3.set_title('Receiver I2 vs Time for Different Decay Rates k_d_ds', fontsize=14)
...
plt.plot(np.array(k_d_ds_values)[valid_indices], 
         np.array(tw50_values)[valid_indices] / 3600,  # Convert to hours
         'o-', linewidth=2, markersize=8)
plt.xlabel('Decay Rate k_d_ds (1/s)', fontsize=12)
plt.ylabel('Time to [I2] = 50 nM (hours)', fontsize=12)
plt.title('Time for I2 to Drop to 50 nM vs k_d_ds', fontsize=14)
...
for k_d_ds in k_d_ds_array:
    ...
    print(f"  k_d_ds = {k_d_ds:.5f} 1/s → [S2 free] = {S2[-1]*1e9:.1f} nM, "
